...
$.fn.slideWiz = function (e) {
  ...
  var getCoverImage = function (position) {
    return $("<div>")
      ...
      .append($("<img>").attr("src", object.file[position].src.cover));
  };
    ...
    if (object.file[position].desc !== false) {
      detail_box.append(
        $("<span>")
          ...
          .html(
            object.file[position].desc.substring(
              0,
    ...
    switch (object.type) {
      case "card":
        if (direction === FORWARD) {
          position =
            getPosition() === object.file.length - 1 ? 0 : getPosition() + 1;
          holder.append(
            getCoverImage(position).css("bottom", "-" + height + "px")
          );
        } else {
          position =
            getPosition() === 0 ? object.file.length - 1 : getPosition() - 1;
        ...
        $(".slide-container .detail-box").css(
          "bottom",
          "-" + container_height + "px"
        );
        ...
        tm = setTimeout(function () {
          ...
          _tm = setTimeout(function () {
            ...
            detail_box.html(getDetail(position));
            ...
            if (object.file[position].src.main !== false) {
              $(".slide-container .main-image-box")
                .html(getMainImage(position))
                .css("top", 50);
            }
      ...
      case "glide":
        if (direction === FORWARD) {
          position =
            getPosition() === object.file.length - 1 ? 0 : getPosition() + 1;
          holder.append(
            getCoverImage(position).css("right", "-" + max_width + "px")
          );
        } else {
          position =
            getPosition() === 0 ? object.file.length - 1 : getPosition() - 1;
          ...
          tm = setTimeout(function () {
            ...
            func_fourth = function () {
              _tm = setTimeout(function () {
                ...
                detail_box.css(
                  "bottom",
                  (container_height - detail_height) / 2
                );
        ...
        tm = setTimeout(function () {
          ...
          func_first = function () {
            if (x < child_length) {
              ...
              $(children[z])
                .addClass(pixel === 0 ? "pixel-scale" : "pixel-rotate")
                .css({
                  width: min_width + "px",
                ...
                .css({
                  ...
                  "margin-top":
                    "-" +
                    (b > 0 ? min_height * b + x_margin_top : x_margin_top) +
                    "px",
                })
                .attr("src", object.file[position].src.cover);
              ...
              children.removeClass(
                pixel === 0 ? "pixel-scale" : "pixel-rotate"
              );
              func_sec();
            }
          };
